...
class Ftc_Metaboxes{
	...
	function add_meta_boxes(){
		$datas = array(
					array(
						'id' 			=> 'page_options'
						,'label' 		=> esc_html__('Page Options', 'ornaldo')
						,'post_type'	=> 'page'
					)
					,array(
						'id'			=> 'testimonial_options'
						,'label'		=> esc_html__('Testimonial Options', 'ornaldo')
						,'post_type'	=> 'ftc_testimonial'
					)
					,array(
						'id'			=> 'team_options'
						,'label'		=> esc_html__('Member Information', 'ornaldo')
						,'post_type'	=> 'ftc_team'
					)
					,array(
						'id'			=> 'portfolio_options'
						,'label'		=> esc_html__('Portfolio Options', 'ornaldo')
						,'post_type'	=> 'ftc_portfolio'
					)
					,array(
						'id'			=> 'portfolio_gallery'
						,'label'		=> esc_html__('Portfolio Gallery', 'ornaldo')
						,'post_type'	=> 'ftc_portfolio'
						,'context'		=> 'side'
						,'priority'		=> 'low'
					)
					,array(
						'id'			=> 'logo_options'
						,'label'		=> esc_html__('Logo Options', 'ornaldo')
						,'post_type'	=> 'ftc_brand'
					)
					,array(
						'id'			=> 'product_options'
						,'label'		=> esc_html__('Product Options', 'ornaldo')
						,'post_type'	=> 'product'
					)
					,array(
						'id'			=> 'post_options'
						,'label'		=> esc_html__('Post Options', 'ornaldo')
						,'post_type'	=> 'post'
					)
					,array(
						'id'			=> 'post_gallery'
						,'label'		=> esc_html__('Post Gallery', 'ornaldo')
						,'post_type'	=> 'post'
						,'context'		=> 'side'
						,'priority'		=> 'low'
					)
					,array(
						'id'			=> 'product_360'
						,'label'		=> esc_html__('Product 360', 'ornaldo')
						,'post_type'	=> 'product'
						,'context'		=> 'side'
						,'priority'		=> 'low'
					)
			...
			switch( $option['type'] ){
				...
				case 'upload':
					...
					$html .= '<div class="ftc-meta-box-field">';
						...
						$html .= '<div class="field">';
							...
							$html .= '<input type="button" class="ftc_meta_box_upload_button" value="'.esc_html__('Select Image', 'ornaldo').'" />';
							$html .= '<input type="button" class="ftc_meta_box_clear_image_button" value="'.esc_html__('Clear Image', 'ornaldo').'" '.($post_meta_value?'':'disabled').' />';
						if( strlen($option['desc']) > 0 ){
							$html .= '<p class="description">'.$option['desc'].'</p>';
						}
						if( $post_meta_value ){
				...
				case 'gallery':
					...
					$html .= '<div class="ftc-meta-box-field ftc-gallery-box">';
						...
						$html .= '<a href="#" class="add-image">'.esc_html__('Add Images', 'ornaldo').'</a>';
					$html .= '</div>';
				break;
