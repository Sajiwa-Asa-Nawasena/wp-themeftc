...
<div class="details-img <?php if($vertical_thumbnail){ echo 'vertical' ;} ?>">
	...
	if ( has_post_thumbnail() ) {
		...
		if( $smof_data['ftc_prod_cloudzoom'] == 1 ){
			...
			$html .= '<a href="' . esc_url( $full_size_image[0] ) . '" class="swipebox" title="'.esc_html($title).'"><span class="full-light">'.esc_html("Fullscreen").'</span>';
			$html .= get_the_post_thumbnail( $post->ID, 'shop_single', $attributes );
		...
		$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'ornaldo' ) );
		$html .= '</a></div>';
	}
	...
	<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?> ftc-grid-main-images" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
		<figure class="woocommerce-product-gallery__wrapper">
			...
			if ( $product->get_image_id() ) {
				...
				$html  = '<div class="woocommerce-product-gallery__image--placeholder">';
				$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'ornaldo' ) );
				$html .= '</div>';
			}
